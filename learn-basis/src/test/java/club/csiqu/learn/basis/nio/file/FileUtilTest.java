...
class FileUtilTest {
    ...
    private static final String FROM_PATH = "D:/file-test/test.txt";
    ...
    void testFileCopy() throws IOException {
        ...
        FileUtil.fileCopyByInputStream(FROM_PATH, "D:/file-test/copy0.txt");
        LOGGER.debug("fileCopyByInputStream cost time:{}", watch);

        watch.reset();
        watch.start();
        FileUtil.fileCopyByBufferedStream(FROM_PATH, "D:/file-test/copy1.txt");
        LOGGER.debug("fileCopyByBufferedStream cost time:{}", watch);

        watch.reset();
        watch.start();
        FileUtil.fileCopyByChannel(FROM_PATH, "D:/file-test/copy2.txt");
        LOGGER.debug("fileCopyByChannel cost time:{}", watch);

        watch.reset();
        watch.start();
        FileUtil.fileCopyByMapped(FROM_PATH, "D:/file-test/copy3.txt");
        LOGGER.debug("fileCopyByMapped cost time:{}", watch);

        watch.reset();
        watch.start();
        FileUtil.fileCopyByTransferTo(FROM_PATH, "D:/file-test/copy4.txt");
        LOGGER.debug("fileCopyByTransferTo cost time:{}", watch);

        watch.reset();
        watch.start();
        FileUtil.fileCopyByFiles(FROM_PATH, "D:/file-test/copy5.txt");
        LOGGER.debug("fileCopyByFiles cost time:{}", watch);

        File file = new File("D:/file-test/copy5.txt");
